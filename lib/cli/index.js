...
module.exports = class CLI {
  ...
  generate (inputfile, outputfile) {
    ...
    let result = JSON.stringify(parse(grammar), undefined, 2);
  ...
  inlineFunctions (result) {
    ...
    parts.forEach((part) => {
      stack.push(part)
      // TODO: do inlineFunctions and inlineRegexes over the parse tree, not the
      // serialized output to avoid issues like this
      if (snippets.length > 0) stack.push(`"code":function($1,$2,$,$ctx){${snippets.shift().replace(/\\\\/g, '\\')}}`)
    })
    return stack.join('')
  }
